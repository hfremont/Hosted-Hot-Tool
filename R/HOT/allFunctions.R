...
getShinyMap <- function(inputSurvey = NULL,
  ...
  switch(inputSurvey,
         "London" = { ts <- readRDS(file = paste0(getHOTdirectory(), "/LTDS.ts.rds")) },
         "France" = { ts <- readRDS(file = paste0(getHOTdirectory(), "/ENTD.fr.ts.rds")) },
         "USA" = {}, # update later once USA integrated
         "Custom" = { ts <- inputCustom })
  
  # Load GIS
  switch(inputSurvey,
         "London" = { GIS.df <- readRDS(file = paste0(getHOTdirectory(), "/LondonGIS.rds")) },
         "France" = { GIS.df <- readRDS(file = paste0(getHOTdirectory(), "/FranceGIS.rds")) },
         "USA" = {}, # update later once USA integrated
  ...
  if( metric == "CRA"){
    ...
    switch(inputSurvey,
           "London" = { filename <- paste0(getHOTdirectory(), "/craTSLondon.rds") },
           "France" = { filename <- paste0(getHOTdirectory(), "/craTSFrance.rds") },
           "USA" = {}, # update later once USA integrated
  ...
  # Everything besides CRA, but written such that this works for CRA in future
    ...
    dir.create(paste0(basepath, "/temp_files"), showWarnings = FALSE)
    
    # Check if local results file exists -- if so, load. If not, compute, load, and store locally.
    filename <- paste0(basepath, "/temp_files/", metric, mode, inputSurvey, ".rds")
...
cleanup <- function(){
  ...
  filenames <- list.files(paste0(getHOTdirectory(), "/temp_files/"), full.names = TRUE)
